...
{
global $d;
$d->Query("UPDATE `module` SET `stat`='0' WHERE `name`='simplelink' LIMIT 1");
print_msg('ماژول با موفقيت غير فعال شد.','Success');
}
function activateop()
{
global $d;
$d->Query("UPDATE `module` SET `stat`='1' WHERE `name`='simplelink' LIMIT 1");
print_msg('ماژول با موفقيت فعال شد.','Success');
}
function installop()
{
global $d;
$q = $d->getrows("SELECT `stat` FROM `module` WHERE `name`='simplelink' LIMIT 1",true);
if($q > 0)
...
(1, 'سيستم مديريت محتواي راش', 'http://rashcms.com/', 'سيستم مديريت محتواي راش', 0),
(2, 'سامانه نظر سنجي راش سي ام اس', 'http://rashcms.ir', 'سامانه نظر سنجي راش سي ام اس', 0),
...
(5, 'انجمن پشتيباني راش سي ام اس', 'http://forum.rashcms.com', 'انجمن پشتيباني راش سي ام اس', 0);");
	...
	$q = $d->Query("INSERT INTO `module` SET `name`='simplelink',`title`='$information[name]',`stat`='0'");
...
print_msg('اين ماژول نصب نشده است يا استاندارد نيست.','Info');
	...
	$q = $d->Query("DELETE FROM `module` WHERE `name`='simplelink' LIMIT 1");
