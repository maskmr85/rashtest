...
{
global $d;
$d->Query("UPDATE `module` SET `stat`='0' WHERE `name`='newsletter' LIMIT 1");
print_msg('ماژول با موفقيت غير فعال شد.','Success');
}
function activateop()
{
global $d;
$d->Query("UPDATE `module` SET `stat`='1' WHERE `name`='newsletter' LIMIT 1");
...
print_msg('اين ماژول قبلا نصب شده است.','Info');
	...
	$q = $d->Query("INSERT INTO `module` SET `name`='newsletter',`title`='$information[name]',`stat`='0'");
...
print_msg('اين ماژول نصب نشده است يا استاندارد نيست.','Info');
	...
	$q = $d->Query("DELETE FROM `module` WHERE `name`='newsletter' LIMIT 1");
